LANGUAGE_NAME := tree-sitter-erpel-script
HOMEPAGE_URL := https://gitlab.com/haasal/tree-sitter-erpel-script
VERSION := 0.1.0
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/erpel-script '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/erpel-script
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/erpel-script
